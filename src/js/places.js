class Place{
    constructor(x, y, isStatic, app, id){
        this.type = "place";
        ...
        this.markinglabel = "";
        this.markingtokens = [];
        this.objektymiesta = this.novy_svg_place(this, app, xmlns, x, y, this.polomer);


    }

    novy_svg_place(element, app, xmlns, x, y, polomer) {
        ...
        app.canvas.add(svgelement);

        this.tokeny(element, app.canvas);
        ...
        svgelement.onmousedown = function () {
            element.onplacedown(element, app, svgelement, svgmeno, labelnode, svgmarking, markingnode, svgzamenom)
        };
    ...
    onplacedown(element, app, svgelement, svgmeno, labelnode, svgmarking, markingnode, svgzamenom) {
        ...
        if (document.getElementById("resetarc").checked) {
            if (kresli_sa_hrana === 0) {
                ...
                bod.x = element.x + arrowHeadSize; //event.pageX - canvas.getBoundingClientRect().left;
                bod.y = element.y; //event.pageY - canvas.getBoundingClientRect().top;
                hranabymove = Arc.novy_svg_temp_arc(canvas, xmlns, element, bod, "reset");
            }
        }
        ...
        if (document.getElementById("inhibitorarc").checked) {
            if (kresli_sa_hrana === 0) {
                ...
                bod.x = element.x + arrowHeadSize; //event.pageX - canvas.getBoundingClientRect().left;
                bod.y = element.y; //event.pageY - canvas.getBoundingClientRect().top;
                hranabymove = Arc.novy_svg_temp_arc(canvas, xmlns, element, bod, "inhibitor");
            }
        }
        ...
        if (document.getElementById("readarc").checked) {
            if (kresli_sa_hrana === 0) {
                ...
                bod.x = element.x + arrowHeadSize; //event.pageX - canvas.getBoundingClientRect().left;
                bod.y = element.y; //event.pageY - canvas.getBoundingClientRect().top;
                hranabymove = Arc.novy_svg_temp_arc(canvas, xmlns, element, bod, "read");
            }
        }
        ...
        if (document.getElementById("position").checked) {
            ...
            if (doit)
                Place.movemiesto(element, novex, novey, objectStore);
        }
    ...
    static movemiesto(miesto, x, y, objectStore) {
        miesto.x = x;
        miesto.y = y;
        ...
        Place.updatepositionmarking(miesto);
        ...
        for (let i = 0; i < objectStore._arcs.length; i++) {
            if (miesto === objectStore._arcs[i].source || miesto === objectStore._arcs[i].target)
                Arc.updatehranusvg(objectStore._arcs[i]);
        }
    }

    static updatepositionmarking(place) {
        var x = place.x;
        var y = place.y;
    ...
    updatemarkingsvg(place) {
        ...
        if (zadane != null) {
            ...
            if (!isNaN(marking) && marking >= 0) {
                ...
                Place.updatetokeny(place);

            }
        }
    }

    static updatemarkings(places) {
        for (let i = 0; i < places.length; i++) {
            Place.updatetokeny(places[i]);
        }
    }

    static updatetokeny(place) {
